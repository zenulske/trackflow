<script>
  ...
  // Defensive Defaults falls Server-Daten fehlen
  let eintraege = $derived(data.eintraege ?? []);
  let verlauf = $derived(data.verlauf ?? []);
  let stats = $derived(data.stats ?? { wertSumme: 0, stundenSumme: 0 });
  let fortschritt = $derived(data.fortschritt ?? { aktuell: 0, prozent: 0 });
    ...
    <div class="card-body">
      <div class="d-flex justify-content-between align-items-center mb-2">
        ...
        <span class="fw-bold" style="color:{data.bereich.farbe};">
          {fortschritt.aktuell} / {data.bereich.zielAnzahl}
          {einheit}
        </span>
      </div>
      <div class="progress mb-1" style="height:8px;">
        <div
          class="progress-bar"
          style="width:{fortschritt.prozent}%;background:{data.bereich.farbe};"
        ></div>
      </div>
      <div class="d-flex justify-content-between mt-1">
        <span class="text-muted small">{fortschritt.prozent}% erreicht</span>
        {#if fortschritt.prozent >= 100}
...
<div class="card border-0 shadow-sm mb-4">
  <div class="card-body">
    ...
    <div class="chart-container">
      {#each verlauf as v (v.start)}
        <div
          class="chart-spalte"
          title="{v.aktuell} {einheit}{data.bereich.zielAnzahl
            ? ` – ${v.prozent}%`
            : ''}"
        >
          ...
          <div class="chart-balken-bereich">
            ...
            <div
              ...
              style="height:{(v.aktuell / maxWert) * 100}%;
                        background:{data.bereich.zielAnzahl && v.erreicht
                ? data.bereich.farbe
        ...
        <div class="d-flex align-items-center gap-1 small text-muted">
          <span
            style="display:inline-block;width:14px;height:10px;background:{data
              ...
              .farbe};border-radius:2px;"
          ></span>
          {einheit} pro {zeitraum}
        </div>
        <div class="d-flex align-items-center gap-1 small text-muted">
          <i class="bi bi-info-circle"></i>
          Setze ein Ziel um den Fortschritt zu sehen
        </div>
      {/if}
    </div>
  </div>
...
<div class="row g-3 mb-4">
  <div class="col-6 col-md-3">
    <div class="card border-0 shadow-sm text-center p-3">
      <div class="fw-bold fs-3">{eintraege.length}</div>
  ...
  <div class="col-6 col-md-3">
    <div class="card border-0 shadow-sm text-center p-3">
      <div class="fw-bold fs-3" style="color:{data.bereich.farbe};">
        {fortschritt.aktuell}
      </div>
      <div class="text-muted small">{einheit} diese {zeitraum}</div>
    </div>
  </div>
  ...
  {#if data.bereich.zielTyp === "wert"}
    <div class="col-6 col-md-3">
      <div class="card border-0 shadow-sm text-center p-3">
        <div class="fw-bold fs-3">{stats.wertSumme}</div>
        <div class="text-muted small">
          {data.bereich.zielEinheit || "Wert"} total
        </div>
      </div>
    </div>
  {:else}
    <div class="col-6 col-md-3">
      <div class="card border-0 shadow-sm text-center p-3">
        <div class="fw-bold fs-3">{stats.stundenSumme}</div>
      ...
      <div class="card-body p-4">
        ...
        {#if eintraege.length > 0}
          <div class="alert alert-warning py-2 small mb-4">
            ...
            <strong>Achtung:</strong> Alle {eintraege.length} Einträge dieses Bereichs
