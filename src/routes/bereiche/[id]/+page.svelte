<script>
  ...
  // maxWert für Chart: höher von Ziel oder höchstem Wert im Verlauf
...
<div class="card border-0 shadow-sm mb-4">
  <div class="card-body">
    <div class="d-flex justify-content-between align-items-center mb-3">
      ...
      {#if data.bereich.zielAnzahl}
        <span class="badge bg-light text-muted small">Ziel: {data.bereich.zielAnzahl} {einheit}</span>
      {/if}
    </div>

    <div class="chart-container">
      {#each data.verlauf as v (v.start)}
        <div class="chart-spalte" title="{v.aktuell} {einheit}{data.bereich.zielAnzahl ? ` – ${v.prozent}%` : ''}">
          <div class="chart-wert" class:erreicht={v.erreicht}>{v.aktuell}</div>
          <div class="chart-balken-bereich">
            <!-- Ziel-Linie nur bei definiertem Ziel -->
            {#if data.bereich.zielAnzahl}
              <div class="chart-ziel-linie"
                   style="bottom:{(data.bereich.zielAnzahl / maxWert) * 100}%;"></div>
            {/if}
            <div class="chart-balken"
                 ...
                 style="height:{(v.aktuell / maxWert) * 100}%;
                        ...
                        border-color:{data.bereich.farbe};"></div>
          </div>
          <div class="chart-label" class:aktuell={v.istAktuell}>{v.label}</div>
        </div>
      {/each}
    </div>
...
<div class="row g-3 mb-4">
  ...
  {#if data.bereich.zielTyp === 'wert'}
    <div class="col-6 col-md-3">
      <div class="card border-0 shadow-sm text-center p-3">
        <div class="fw-bold fs-3">{data.stats.wertSumme}</div>
        <div class="text-muted small">{data.bereich.zielEinheit || 'Wert'} total</div>
      </div>
    </div>
  {:else}
    <div class="col-6 col-md-3">
      <div class="card border-0 shadow-sm text-center p-3">
        <div class="fw-bold fs-3">{data.stats.stundenSumme}</div>
  ...
  {:else}
    <ul class="list-group list-group-flush">
      {#each data.eintraege as e (e._id)}
        <li class="list-group-item d-flex justify-content-between align-items-center py-3 gap-3 flex-wrap">
          ...
          <div class="d-flex gap-1">
            <a href="/eintrag/{e._id}/bearbeiten" class="btn btn-sm btn-outline-secondary">
              Bearbeiten
            </a>
            <button class="btn btn-sm btn-outline-danger" onclick={() => eintragModalOeffnen(e)}>
              Löschen
            </button>
          </div>
        </li>
    ...
    <div class="card border-0 shadow-lg">
      <div class="card-body p-4">
        ...
        <div class="d-flex gap-2 justify-content-end">
          <button class="btn btn-outline-secondary" onclick={eintragModalSchliessen} disabled={laden}>Abbrechen</button>
  ...
  <div class="modal-dialog-custom" role="dialog" aria-modal="true">
    <div class="card border-0 shadow-lg">
      <div class="card-body p-4">
        ...
        {/if}
        <div class="d-flex gap-2 justify-content-end">
          <button class="btn btn-outline-secondary" onclick={bereichModalSchliessen} disabled={laden}>Abbrechen</button>
...
<style>
  .chart-container { display: flex; align-items: flex-end; gap: 12px; height: 180px; padding: 0 4px; }
  .chart-spalte { flex: 1; display: flex; flex-direction: column; align-items: center; height: 100%; }
  .chart-wert { font-size: 12px; font-weight: 600; color: #6A6860; margin-bottom: 4px; height: 18px; }
  .chart-wert.erreicht { color: #1C1B18; }
  .chart-balken-bereich {
    flex: 1; width: 100%; position: relative; display: flex;
    align-items: flex-end; border-bottom: 1px solid #E8E6E2;
  }
  .chart-balken {
    width: 100%; border-radius: 4px 4px 0 0; border: 1px solid;
    transition: height .3s ease; min-height: 2px;
  }
  .chart-balken.aktuell { box-shadow: 0 0 0 2px white, 0 0 0 3px currentColor; }
  .chart-ziel-linie {
    position: absolute; left: -4px; right: -4px; height: 0;
    border-top: 1.5px dashed #9A9890; z-index: 1;
  }
  .chart-label { font-size: 11px; color: #9A9890; margin-top: 6px; font-weight: 500; }
  .chart-label.aktuell { color: #1C1B18; font-weight: 700; }
  ...
  .modal-dialog-custom {
    ...
    width: 90%; max-width: 460px; z-index: 1050; animation: slideIn .2s ease;
  }
