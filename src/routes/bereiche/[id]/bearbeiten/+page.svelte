<script>
  ...
  function waehlefarbe(f) { farbe = f.farbe; farbeHell = f.hell; }

  function setzeTyp(typ) {
    zielTyp = typ;
    if (typ === 'eintraege')      zielEinheit = 'Einträge';
    else if (typ === 'minuten')   zielEinheit = (zielEinheit === 'Std.' ? 'Std.' : 'Min.');
    else if (typ === 'wert') {
      // Bei Wechsel zu wert: Standard-Werte zurücksetzen
      if (zielEinheit === 'Einträge' || zielEinheit === 'Min.' || zielEinheit === 'Std.') {
        zielEinheit = '';
      }
    }
  }
...
<svelte:head><title>{data.bereich.name} bearbeiten – TrackFlow</title></svelte:head>
...
<div class="card border-0 shadow-sm" style="max-width:520px;">
  <div class="card-body p-4">
    {#if form?.fehler}<div class="alert alert-danger py-2 mb-3">{form.fehler}</div>{/if}
    ...
    <form method="POST" use:enhance>
      ...
      <input type="hidden" name="zielEinheit" value={zielEinheit} />

      <div class="mb-3">
        <label for="name" class="form-label fw-medium small">Name *</label>
        <input id="name" name="name" type="text" class="form-control" bind:value={name} required />
      </div>
      ...
      <div class="mb-3">
        ...
        <div class="d-flex flex-wrap gap-2">
          {#each farben as f}
            <button type="button" onclick={() => waehlefarbe(f)} class="rounded-circle border-0"
                    style="width:38px;height:38px;background:{f.farbe};outline:{f.farbe === farbe ? '3px solid #212529' : 'none'};outline-offset:2px;cursor:pointer;">
            </button>
          {/each}
        </div>
      </div>
      ...
      <div class="rounded-3 p-3 d-flex align-items-center gap-3 mb-4"
        ...
        <span class="fw-semibold" style="color:{farbe};">{name || 'Bereich'}</span>
      </div>
      ...
      <div class="border rounded-3 p-3 mb-4">
        ...
        <div class="row g-2 mb-3">
          <div class="col">
            ...
            <input type="number" name="zielAnzahl" class="form-control"
        ...
        <div class="mb-2">
          ...
          <select name="zielZeitraum" class="form-select" bind:value={zielZeitraum}>
