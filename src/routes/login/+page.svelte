<script>
  ...
  let { form } = $props();
  let passwortGeaendert = $derived(
    $page.url.searchParams.get("passwort") === "geaendert",
  );
</script>
...
<svelte:head><title>Anmelden – TrackFlow</title></svelte:head>
    ...
    <div class="card-body p-4 p-md-5">
      <div class="d-flex align-items-center gap-2 mb-4">
        <div
          ...
          style="width:36px;height:36px;background:#212529;font-size:13px;"
        >
          TF
        </div>
      ...
      {#if form?.fehler}
        <div class="alert alert-danger py-2">{form.fehler}</div>
      {/if}
      ...
      <form method="POST" use:enhance>
        ...
        <div class="mb-4">
          <label for="passwort" class="form-label fw-medium small"
            >Passwort</label
          >
          <input
            id="passwort"
      ...
      <p class="text-center text-muted small mt-4 mb-0">
        Noch kein Konto? <a href="/register" class="text-dark fw-medium"
